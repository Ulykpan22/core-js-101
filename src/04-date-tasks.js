...
function timeSpanToString(startDate, endDate) {
  ...
  let milSec = date.getUTCMilliseconds().toString();
  if (milSec.length === 2) {
    milSec = `0${milSec}`;
  } else if (milSec.length === 1) {
    milSec = `00${milSec}`;
  }
  return `${getzero(date.getUTCHours())}:${getzero(date.getUTCMinutes())}:${getzero(date.getUTCSeconds())}.${milSec}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let angle = Math.abs((H * 60 + M) * 0.5 - M * 6);
